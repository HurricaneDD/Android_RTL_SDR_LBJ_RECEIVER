...
- Github Release 下载：[<img alt="Get it on GitHub" src="media/github.png" width="200px">](https://github.com/HurricaneDD/Android_RTL_SDR_LBJ_RECEIVER/releases)
...
- 可能因部分既有线动集列车不发送LBJ信号，似乎无法预警部分既有线动集列车。
- 部分动集列车信息可能接收不全或解析异常，*例如D12次解析为12次。*
- 可能因大部分高铁列车不发送LBJ信号，似乎无法预警高铁列车。
- 部分城际铁路线上运行的高速动车组列车可能不发送LBJ信号，因此无法收到相关预警。*在南京站观测到：在沪宁城际旁，没有接收到一列D/G头列车的信号，仅接收到一列C头列车信号*。
